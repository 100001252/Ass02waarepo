...
public class Ass02BookSoap {
    ...
    public String getAllBook()
    {
        ...
        try{
 	       ...
 	       while(myRs.next())
 	       {
 	    	   result+=""+myRs.getInt("id")+"/#"+myRs.getString("title")+"/#"+myRs.getString("authorList")+"/#"+myRs.getString("isbn")+"/#"+myRs.getString("publisher")+"/#"+myRs.getString("publishDate")+"/#"+myRs.getString("status")+"##";
 	    //	   lstBook.add(new Book(myRs.getInt("id")/#myRs.getString(2),myRs.getString(3),myRs.getString(4),myRs.getString("publisher"),myRs.getString("publishDate"),myRs.getString("status"),myRs.getInt("student_id")));
 	       }
    ...
    public String getRecordsStudent(Integer studentId)
    {
        ...
        try{
 	       ...
 	       while(myRs.next())
 	       {
 	    	   result+=""+myRs.getInt("id")+","+myRs.getInt("student_id")+","+myRs.getInt("book_id")+","+myRs.getString("action")+"##";
 	    //	   lstBook.add(new Book(myRs.getInt("id"),myRs.getString(2),myRs.getString(3),myRs.getString(4),myRs.getString("publisher"),myRs.getString("publishDate"),myRs.getString("status"),myRs.getInt("student_id")));
 	       }
				    
				    System.out.println("myNotegetrecordssttt specific student records \n"+result);
				    return result;
        }catch(Exception e)
        {
        			return e.getMessage();
    ...
    /**get records of one specific student just different version and it gives more infor
     * 
     * @param studentId
     * @return student details in String format
     */
    public String getRecordsStudent2(Integer studentId)
    {
        ...
        try{
 	       ...
 	       while(myRs.next())
 	       {
 	    	   result+=""+myRs.getInt("br.id")+","+myRs.getInt("br.student_id")+","+myRs.getInt("br.book_id")+","+myRs.getString("bk.title")+","+myRs.getString("bk.status")+","+myRs.getString("action")+"##";
 	    //	   lstBook.add(new Book(myRs.getInt("id"),myRs.getString(2),myRs.getString(3),myRs.getString(4),myRs.getString("publisher"),myRs.getString("publishDate"),myRs.getString("status"),myRs.getInt("student_id")));
 	       }
    ...
    public String getBook(Integer bookId)
    {
        ...
        try{
 	       ...
 	       while(myRs.next())
 	       {
 	    	   result+=""+myRs.getInt("id")+","+myRs.getString("title")+","+myRs.getString("authorList")+","+myRs.getString("publisher")+","+myRs.getString("publishDate")+","+myRs.getString("status")+"##";
 	    //	   lstBook.add(new Book(myRs.getInt("id"),myRs.getString(2),myRs.getString(3),myRs.getString(4),myRs.getString("publisher"),myRs.getString("publishDate"),myRs.getString("status"),myRs.getInt("student_id")));
 	       }
     ...
     */
    public String updateBook(Integer bookid,String title,String authorList,String isbn,String publishDate,String publisher,String status) throws BookException
    {
    	String result="";
    	try{
    		DbBook dbbook= new Db().new DbBook();
    	if(!dbbook.findBook(bookid))
    		throw new BookException("there is no book with id="+bookid+" in our database");
    	if(!Book.isStatusValid(status))
    		throw new BookException("status is not valid. it has to be either(available,on loan,back order)");
    	if(!Book.isIsbnValid(isbn))
            throw new BookException("isbn has to be 13 or 10 digit number");
    	
    	
    	
        dbbook.updateBook(bookid,title,authorList,isbn,publishDate,publisher,status);
   ...
   /**function to add book with all details
     ...
     * @throws BookException
     */
    public String addBook(String bookTitle,String authorList,String isbn,String publisher,String 
            publishDate,String status) throws BookException
    {
    	try{
    		DbBook dbbook= new Db().new DbBook();
        ...
        if(dbbook.isBookWithIsbnExist(isbn))
        	throw new BookException("this isbn already exists in our system");
        
        
        Book objb=new Book( bookTitle, authorList, isbn, publisher, publishDate, status,0);
      ...
      //  readAllBooksFromDb();
        String result="Book has been added successfully. book Id is:"+id;
    ...
    public String addBook2(String bookTitle,String isbn) throws BookException
    {
        ...
        try{
        	...
        	if(!Book.isIsbnValid(isbn))
        	 ...
        	 if(dbbook.isBookWithIsbnExist(isbn))
             	throw new BookException("this isbn already exists in our system");
        Book objb=new Book( bookTitle, "my author list", isbn, "publisher", "22/03/2016", "available",0);
...
/** action in database has to be borrow request or return (although if a student return the book we delete the that record)
 * 
 * @param action
 * @return
 */
    private boolean isActionValid(String action)
    {
    	...
    	if(action.equals("on loan") || action.equals("back order") || action.equals("available") || action.equals("delete") )
    		result=true;
    	return result;
    }
    /**update current student record search by studentid and bookid and change action(e.g to returned)
     * it just change the action if it exist
     * @param studentId
     * @param bookId
     * @param action
     ...
     * @throws BookException
     */
    public String updateStudentrecords(Integer studentId,Integer bookId,String action) throws BookException
    {
    	...
    	try{
        	...
        	if(!isActionValid(action))
        		throw new BookException("action has to be (borrow,request or returned)");
    ...
    /** function to insert student borrow record. so they can update if they have existing record
     * 
     * @param studentId
     * @param bookId
     * @param action
     ...
     * @throws BookException
     */
    public String insertStudentrecords(Integer studentId,Integer bookId,String action) throws BookException
    {
    	...
    	try{
        	...
        	if(!dbbook.findBook(bookId))
        		throw new BookException("there is no book with id="+bookId+" in our database");
        	if(!isActionValid(action))
        		throw new BookException("action has to be (on loan,back order or available)");
        	if(dbStudent.isRecordExist(studentId, bookId))
        		throw new BookException("already there is a record for student with this id="+studentId+" and book id="+bookId);
...
String result="";
     ...
     * @throws BookException
     */
    public String setBookStatus(String bookId,String bstatus) throws BookException
    {
    	try{
    		DbBook dbbook= new Db().new DbBook();
        ...
        return " Book id "+bookId+" status has been updated to "+bstatus;
    	}catch(Exception e)
    	{
    		return e.getMessage();
    	}
