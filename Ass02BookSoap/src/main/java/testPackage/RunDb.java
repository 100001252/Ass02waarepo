...
public static void testUniLibaddBook(String title,String isbn){
	...
	ubook.addBook(title, "authortest", isbn, "testPublisher", "22/08/1980", "available");
	}catch(Exception e)
	{
		System.out.println(e.getMessage());
	}
...
public static void testUniLibupdateBook(String title,String isbn){
	try{
	Ass02BookSoap ubook=new Ass02BookSoap();
	ubook.updateBook(1, title, "", isbn, "nothin","publisher", "on loan");
	}catch(Exception e)
	{
		System.out.println(e.getMessage());
	}
...
public static void testinsertBook()
{
	try{
	   ...
	   Db.DbBook dbBook=new Db("").new DbBook();
	   Book objBook=new Book( "title", "author list", "1234567892", "mypublisher", "22/09/2008", "available",1);
	...
	public static void testReadBook()
	{
		try{
	       ...
	       ResultSet myRs=dbBook.getAllBooks();
	        ...
	        while(myRs.next()){
	            System.out.println("book name:"+myRs.getString("title")+" isbn:"+myRs.getString("isbn"));
	        }
		}catch(Exception e)
		{
			System.out.println("11122");
